...
import { Package, Lock, Mail, ArrowRight, Sun, Moon, UserPlus, ArrowLeft } from 'lucide-react';
...
const Register: React.FC<RegisterProps> = ({ onRegister, onBackToLogin, isDarkMode, toggleTheme }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError('Erro ao criar conta. Verifique se o Supabase está configurado.');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="min-h-screen flex items-center justify-center bg-[#f8fafc] dark:bg-slate-950 p-4 transition-colors duration-300">
            ...
            <div className="w-full max-w-md animate-in fade-in zoom-in duration-500">
                <div className="text-center mb-10">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 font-medium">Criar Nova Conta</p>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] shadow-2xl shadow-slate-200 dark:shadow-none border border-slate-100 dark:border-slate-800">
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div>
                            <label className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-widest mb-2 block">Seu Nome</label>
                            <div className="relative group">
                                <UserPlus className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-yellow-500 transition-colors" size={20} />
                                <input
                                    required
                                    type="text"
                                    value={name}
                        ...
                        <div>
                            <label className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-widest mb-2 block">Senha</label>
                            <div className="relative group">
                        ...
                        <div>
                            <label className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-widest mb-2 block">Confirmar Senha</label>
                            <div className="relative group">
                                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-yellow-500 transition-colors" size={20} />
                                <input
                                    required
                    ...
                    <div className="mt-8 pt-8 border-t border-slate-100 dark:border-slate-800 text-center">
                        <button
                            ...
                            className="text-sm text-slate-600 dark:text-slate-400 font-medium hover:text-yellow-600 dark:hover:text-yellow-500 transition-colors flex items-center justify-center gap-2 mx-auto"
                        >
                            <ArrowLeft size={16} />
